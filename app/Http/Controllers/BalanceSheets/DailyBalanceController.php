...
{
    protected $viewPath = 'balancesheet.daily-balance';
    protected $routePath = 'balancesheet/daily-balance';
    protected $translationPrefix = 'balancesheet.balance.';
    protected $model = 'App\Models\DailyBalanceSheet';
    ...
    {
        $this->editFields = ['date_added', 'type_id', 'target_id', 'product_name', 'amount', 'unit_price'];
        $this->updateFields = ['date_added', 'type_id', 'target_id', 'amount', 'unit_price'];
    ...
    public function index()
    {
        ...
        $page = (object) ['title' => __($this->translationPrefix . 'DailyEntries'), 'route' => '', 'routeCreate' => route($this->routePath . '/create'),
        ...
        return view($this->viewPath . '.list', compact('page', 'breadcrumbPath', 'balanceTypeSelectOptions', 'targetSelectOptions', 'userDateFormat'));
    }
    ...
    function list(Request $request) {
        ...
        $data = $this->model::with(['productInfo', 'typeInfo', 'targetInfo'])
            ->select(['id', 'date_added', 'type_id', 'product_id', 'target_id', 'amount', 'unit_price', 'price', 'updated_at'])
        ...
        return Datatables::of($data)
            ...
            ->filterColumn('date_added', function ($query, $keyword) {
                $this->applyQueryCustomFilter($query, $keyword, 'date_added');
            })
            ...
            ->addColumn('date_added', function ($item) {
                return DateUtils::mysqlToUserDate($item->date_added, $this->userSettings->date_format_php);
            })
    ...
    public function create()
    {
        ...
        if (empty(request()->old())) {
            $data->type_id = BalanceType::Debit;
        }
        ...
        $page = (object) ['title' => __($this->translationPrefix . 'DailyEntries'), 'name' => __($this->translationPrefix . 'CreateNew'),
        ...
        return view($this->viewPath . '.edit',
            compact('data', 'page', 'breadcrumbPath', 'balanceTypeSelectOptions', 'targetSelectOptions', 'userDateFormat'));
    }
    ...
    public function edit($id)
    {
        ...
        // if data comes from db, format it
        if (empty(request()->old())) {
            $data->date_added = DateUtils::mysqlToUserDate($data->date_added, $this->userSettings->date_format_php);
        }
        Form::updateModelFromRequest(request()->old(), $data, $this->editFields);

        $balanceTypeSelectOptions = SelectUtils::getBalanceTypeSelectOptions($data->type_id);
        $targetSelectOptions = SelectUtils::getTargetSelectOptions($data->target_id);
        $page = (object) ['title' => __($this->translationPrefix . 'DailyEntries'), 'name' => __('tables.Edit') . ': ' . $data->name,
        ...
        $breadcrumbPath = $this->routePath;
        return view($this->viewPath . '.edit', compact('data', 'page', 'breadcrumbPath', 'balanceTypeSelectOptions', 'targetSelectOptions', 'userDateFormat'));
    }
    ...
    private function getItemForEdit($itemId)
    {
        $data = $this->model::with(['productInfo'])
            ->select(['id', 'date_added', 'type_id', 'product_id', 'target_id', 'amount', 'unit_price', 'updated_at'])
            ->where('user_id', $this->userId)
            ->findOrFail($itemId);
    ...
    {
        $fieldList = ['id', 'date_added', 'type_name', 'product_name', 'target_name', 'amount', 'unit_price', 'price'];
        $columnList = [__('tables.Id'), __('tables.Date'), __('tables.Month'), __('tables.Type'), __('tables.Product'), __('tables.Target')
    ...
    private function createItem(Request $request)
    {
        ...
        $item->date_added = DateUtils::userToMysqlDate($item->date_added, $this->userSettings->date_format_php);
